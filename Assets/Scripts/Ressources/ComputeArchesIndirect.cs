...
public class ComputeArchesIndirect : MonoBehaviour
{
    ...
    public unsafe struct CurveDataSSBO
    {
        ...
        public static CurveDataSSBO FromCurve(Vector3[] curvePoints)
        {
            ...
            if (curvePoints.Length > MAX_POINTS)
                throw new ArgumentException($"Trop de points ({curvePoints.Length}). Max autorisé = {MAX_POINTS}");
        ...
        // ✅ Exporte le contenu vers un tableau GPU-compatible
    ...
    public unsafe struct CurveDataSSBO
    {
        ...
        public fixed float positions[1000 * 4]; // Tableau FIXE pour �tre blittable
        ...
        public static CurveDataSSBO FromCurve(Vector3[] curvePoints)
        {
            ...
            for (int i = 0; i < curvePoints.Length; i++)
            {
                ...
                data.positions[i * 4 + 1] = curvePoints[i].y;
                data.positions[i * 4 + 2] = curvePoints[i].z;
                data.positions[i * 4 + 3] = 1.0f; // Coordonn�e homog�ne
            }
...
public ComputeArchesIndirect(ShaderWatcher shaderwatch, ShaderLibrary shaderLibrary)
    ...
    void InitBuffers()
    {
        ...
        */
        ...
        //Debug.Log($"[Init] Matrix4x4 stride = {s1}"); // Devrait être 64
        ...
        */
        ...
        // Initialisation avec des vecteurs (tout à zéro)
    ...
    public void DispatchIndirect()
    {
        ...
        {
            Debug.LogError("ComputeShader non assigné !");
            return;
        }

        int kernel = computeShader.FindKernel("CSMain");

        // ⚠️ Assurez-vous que le buffer drawIndirectCommandBuffer contient bien un dispatch command
    ...
    void ComputeUpdated()
    {
        ...
        // Lier les buffers au mat�riel et ex�cuter le rendu indirect
